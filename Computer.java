...
public class Computer {
	...
	public boolean findPoint(Chess chess, BoardCaro boardCaro) {
		Chess chessBoard = new Chess(); // Khởi tạo một đối tượng thế cờ [5][5] để lưu từng ma trận [5][5] trên bàn cờ
			...
			for (int col = 1; col <= 16; col++) {
				for (int i = 0; i < 5; i++) {
					for (int j = 0; j < 5; j++) {
						...
						// [5][5] trên bàn cờ
						chessBoard.chess[i][j] = boardCaro.board[row + i][col + j];
					}
				}
				if (compareChess(chess, chessBoard)) { // nếu ma trận [5][5] của thế cờ giống ma trận [5][5] trên bàn cờ
	...
	public void playCom() {
		...
		if (pointD.isNull()) { // Kiểm tra điểm máy cần đánh có phù vs thế cờ nào trong list hay không
			...
			int x = 0, y = 0; // Khởi tạo các biến hàng ngang x, cột dọc y
			while (true) {
